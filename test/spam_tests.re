...
[@deriving show]
type available_commands = list([ | `DeleteMessage(int) | `KickUser(int)]);
...
let asset_ban_message = message_json => {
  ...
  let actual =
    try_ban(
      {as _; pub is_admin = false; pub state = state},
...
let () = {
  ...
  let samples =
    ...
    |> List.map((s, ()) => s |> Base64.decode_exn |> asset_ban_message)
    |> List.map(f => test_case("test spam ban", `Quick, f));
