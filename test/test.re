...
type env('f, 'eff) = {
  func: ('f, Message.message) => list('eff),
  is_admin: bool,
  state,
  mention_user_id: option(int),
  message_date: int,
  is_reply: bool,
};
...
let run_test = (env, expected) => {
  let actual =
    env.func(
      {as _; pub is_admin = env.is_admin; pub state = env.state},
      Message.create(
        ~message_id=100,
        ~date=env.message_date,
        ~from=Some(User.create(~id=400, ~first_name="", ())),
        ~entities=
          switch (env.mention_user_id) {
          | Some(user_id) =>
            Some([
...
let call_try_ban_by_admin = () =>
  run_test(
    {...empty_env, func: try_ban, is_admin: true, is_reply: true},
    [`DeleteMessage(200), `KickUser(300), `DeleteMessage(100)],
  );

let call_try_ban_with_no_permission = () =>
  run_test({...empty_env, func: try_ban}, [`DeleteMessage(100)]);

let add_trusted_user_not_admin = () =>
  run_test({...empty_env, func: add_trusted_user}, [`DeleteMessage(100)]);

let add_trusted_user_no_reply = () =>
  run_test(
    {...empty_env, func: add_trusted_user, is_admin: true},
    [
      `DeleteMessage(100),
...
let remove_trusted_user_not_admin = () =>
  run_test(
    {...empty_env, func: remove_trusted_user},
    [`DeleteMessage(100)],
  );

let remove_trusted_user_no_reply = () =>
  run_test(
    {...empty_env, func: remove_trusted_user, is_admin: true},
    [
      `DeleteMessage(100),
      `SendMessage("Пользователь не указан"),
    ],
...
let remove_trusted_user = () =>
  run_test(
    ...
    [
      `DeleteMessage(100),
      `UpdateState([
        StateEvents.TrustedUserDeleted({chat_id: 500, user_id: 200}),
      ]),
    ],
  );
